...
const createUserToken = async (payload) =>
  await jwt.sign(payload, process.env.SECRET, { expiresIn: 60 ** 2 });

router.post("/", async (req, res) => {
  const userData = req.body;
  if (!userData || !userData.username || !userData.password) {
    return res.status(400).json({ error: "Missing setUser data" });
  }
  const user = await User.findOne({ username: userData.username });
  const dataMatches = user
  ...
  console.log("Successful setUser for", user.username);
